/* 
 * arm7 wifi code from DSWifi lib by Stephen Stair
...
int Wifi_SendSharedKeyAuthPacket2(int, u8 *);
...
void GetWfcSettings(void) {
	...
	c=0;
	wfcBase = ReadFlashBytes(0x20, 2) * 8 - 0x400;
	for(i=0;i<3;i++) WifiData->wfc_enable[i]=0;
	for(i=0;i<3;i++) {
		read_firmware(wfcBase +(i<<8),(char *)data,256);
...
u16 Wifi_MACRead(ulong MAC_Base, ulong MAC_Offset) {
	MAC_Base += MAC_Offset;
	if(MAC_Base>=(WIFI_REG(0x52)&0x1FFE)) MAC_Base -= ((WIFI_REG(0x52)&0x1FFE)-(WIFI_REG(0x50)&0x1FFE));
	return WIFI_REG(0x4000+MAC_Base);
}

void Wifi_MACCopy(u16 * dest, ulong MAC_Base, ulong MAC_Offset, ulong length) {
	int endrange,subval;
	int thislength;
...
void Wifi_MACWrite(u16 * src, ulong MAC_Base, ulong MAC_Offset, ulong length) {
	int endrange,subval;
	int thislength;
	endrange = (WIFI_REG(0x52)&0x1FFE);
	subval=((WIFI_REG(0x52)&0x1FFE)-(WIFI_REG(0x50)&0x1FFE));
...
void Wifi_Interrupt(void*) {
	...
	do {
		WifiData->debug[6]++;
		WifiData->debug[1] = W_IE;
		WifiData->debug[2] = W_IF;
		
		intrclear(WIFIbit, 0);	// now that we've cleared the wireless IF, kill the bit in regular IF.
		wIF= W_IE & W_IF;
...
void Wifi_Update() {
	...
	/* TODO debugging stats */
	WifiData->debug[3] = WIFI_REG(0xA8);
	WifiData->debug[4] = WIFI_REG(0xB8);
}
...
void Wifi_Init(u32 wifidata)
{
	...
	*((volatile u16 *)0x04000206) = 0x30;	// ???
	...
	memset(WifiData->ssid7, 0, sizeof(WifiData->ssid7));
	memset(WifiData->wepkey7, 0, sizeof(WifiData->wepkey7));
